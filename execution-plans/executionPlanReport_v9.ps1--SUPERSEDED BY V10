...
"@
  ...
  if ($chosenStmt) {
    $chosenCERow = @($ceModelInfo | Where-Object { [string]$_.StatementId -eq [string]$chosenStmt.StatementId }) | Select-Object -First 1
...
if ($waitNodes.Count -gt 0) {
    foreach ($w in $waitNodes) {
        $waitStatsReport += [pscustomobject]@{
            ...
            AvgWaitMs   = if ([int]$w.GetAttribute("WaitCount") -gt 0) {
                              [math]::Round(([double]$w.GetAttribute("WaitTimeMs") / [int]$w.GetAttribute("WaitCount")), 3)
                          } else { 0 }
        }
    }
