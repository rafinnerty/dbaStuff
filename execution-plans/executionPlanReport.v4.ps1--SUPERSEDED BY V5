 ...
 # Hybrid scoping: Returns direct objects if present. Otherwise, returns downstream objects as "(via: ...)".
  ...
  $operatorRows = @(
    foreach ($op in $relOps) {
      ...
      if ($DebugSargability) {
        # Only output the debug block if at least one predicate or result exists
        if (-not [string]::IsNullOrWhiteSpace($predCtx) -or 
            -not [string]::IsNullOrWhiteSpace($access.SeekPredicate) -or 
            -not [string]::IsNullOrWhiteSpace($access.Residual) -or 
            -not [string]::IsNullOrWhiteSpace($nonSarg)) {
