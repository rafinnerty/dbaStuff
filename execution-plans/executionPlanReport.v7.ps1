<#
    ...
    Purpose:        Based upon a list of SQL Server instances:
                    1. Write all instance-level role memberships to an "All Instance Perms" worksheet.
                    2. Write all database-level permissions to an "All DBs Perms" worksheet.
                    3. Zip the resulting spreadsheet and remove the unzipped copy.
                    Spreadsheet is auto-width and has filters on every column.
    Changelog:      1.2 - Restored corrupted permission identifiers (database_permissions /
                          permission_name / [Permission Type] / [Permission State]); switched
                          accumulators to List[object]; added error handling on the main query
                          and the zip step; removed dead code ($AdDomainPrefix, $adNames,
                          @HideDatabaseDiagrams).
...
# initialize list that holds PS objects for instance-level permissions details
$instanceArray = [System.Collections.Generic.List[object]]::new()

# initialize list that holds PS objects for database-permissions details
$dbArray = [System.Collections.Generic.List[object]]::new()
...
{
# Progress output for interactive call.
Write-Host ''
...
$instanceRoles = (Get-DbaServerRole -SqlInstance $instance) | Sort-Object name -Unique
    ...
    $instanceRoleMembers = (Get-DbaServerRoleMember -SqlInstance $instance -serverRole $instancerole.name) | Sort-Object name -Unique
        ...
        foreach ($instanceRoleMember in $instanceRoleMembers)
        {
            ...
            # Add a row for this instance-role membership.
            $instanceArray.Add([pscustomobject]@{
                'Instance'             = $instance
                'Instance Role'        = $instanceRole.Name
...
try {
    $dbRows = @(Invoke-DbaQuery -SqlInstance $instance -Database $database.Name -Query $tSql -EnableException)
} catch {
    Write-Warning "Permissions query failed on [$instance].[$($database.Name)]: $($_.Exception.Message)"
    $dbRows = @()
}
...
if ($dbRows) { $dbArray.AddRange([object[]]$dbRows) }
...
#       (not SQL / internal / system principals).
# We export two AD-derived strings (matching the code below):
...
# Zip-up the spreadsheet, then remove the unzipped copy only if the zip succeeded.
Write-Host "`nZipping spreadsheet ..."
$zipFile = "$path.zip"
try {
    Compress-Archive -LiteralPath $path -CompressionLevel Optimal -DestinationPath $zipFile -Force
    Write-Host "Dropping unzipped spreadsheet ..."
    Remove-Item -LiteralPath $path -Force
} catch {
    Write-Warning "Failed to compress '$path': $($_.Exception.Message). Leaving the .xlsx in place."
}
