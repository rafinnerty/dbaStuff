...
function New-RemediationScript {
    ...
    $tsql = @{
        'BACKUP_FULL_STALE' = { param($f) $db = Get-TsqlBracketName $f.ObjectName
            "BACKUP DATABASE $db TO DISK = N'<backup-path>\$($f.ObjectName)_Full.bak' WITH CHECKSUM, COMPRESSION, STATS = 10;" }
        'BACKUP_LOG_STALE' = { param($f) $db = Get-TsqlBracketName $f.ObjectName
            "BACKUP LOG $db TO DISK = N'<backup-path>\$($f.ObjectName)_Log.trn' WITH CHECKSUM, COMPRESSION;" }
        ...
        'DB_STATE_BAD' = { param($f) $db = Get-TsqlBracketName $f.ObjectName
            "-- Diagnose before acting - state, recent errors, and why recovery is not completing:`r`nSELECT name, state_desc, user_access_desc FROM sys.databases WHERE name = N'$($f.ObjectName)';`r`n-- If RECOVERY_PENDING due to missing files, fix the file paths/permissions then:`r`n-- ALTER DATABASE $db SET ONLINE;" }
        ...
        'DB_ORPHANED_USER' = { param($f)
            "-- In the affected database (see Detail), re-map or drop the orphaned user:`r`n-- ALTER USER [$($f.ObjectName)] WITH LOGIN = [$($f.ObjectName)];  -- if the login exists`r`n-- DROP USER [$($f.ObjectName)];                                   -- if stale (REVIEW first)" }
        ...
        'SEC_STARTUP_PROC' = { param($f)
            "-- REVIEW & UNCOMMENT - verify the startup procedure is not required:`r`n-- EXEC sp_procoption @ProcName = N'$($f.ObjectName)', @OptionName = 'startup', @OptionValue = 'off';" }
        ...
        'AGENT_JOB_NO_OWNER' = { param($f)
            "EXEC msdb.dbo.sp_update_job @job_name = N'$($f.ObjectName)', @owner_login_name = N'sa';" }
        'AGENT_JOB_NO_NOTIFY' = { param($f)
            "EXEC msdb.dbo.sp_update_job @job_name = N'$($f.ObjectName)', @notify_level_email = 2, @notify_email_operator_name = N'<operator-name>';" }
        'AGENT_JOB_FAILED' = { param($f)
            "-- Failure detail for this job (most recent runs first):`r`nEXEC msdb.dbo.sp_help_jobhistory @job_name = N'$($f.ObjectName)', @mode = 'FULL';" }
        ...
        'DB_FILE_MAXSIZE_CAP' = { param($f)
            ...
            "-- REVIEW & UNCOMMENT - raising vs removing the cap is a capacity decision:`r`n-- ALTER DATABASE $(Get-TsqlBracketName $dbPart) MODIFY FILE (NAME = N'$filePart', MAXSIZE = UNLIMITED);" }
        'DB_FILE_AUTOGROW_OFF' = { param($f)
            $dbPart = ($f.ObjectName -split '\.', 2)[0]; $filePart = ($f.ObjectName -split '\.', 2)[1]
            "-- REVIEW & UNCOMMENT - only if the no-autogrow policy is NOT deliberate pre-sizing:`r`n-- ALTER DATABASE $(Get-TsqlBracketName $dbPart) MODIFY FILE (NAME = N'$filePart', FILEGROWTH = 256MB);" }
        'DB_LOG_OUTSIZED' = { param($f) $db = Get-TsqlBracketName $f.ObjectName
            "-- Find what is pinning the log before shrinking anything:`r`nSELECT name, log_reuse_wait_desc FROM sys.databases WHERE name = N'$($f.ObjectName)';`r`n-- If log_reuse_wait_desc = LOG_BACKUP, take log backups; only then consider a one-off DBCC SHRINKFILE on the log." }
        'ERRORLOG_IO' = { param($f)
